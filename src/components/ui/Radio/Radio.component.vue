...
const boxedClasses = computed(() =>
  twMerge(
    "items-start p-4 rounded-sm border",
    model.value
    ...
    <template v-if="inputPosition === 'left'">
      <div class="relative flex items-center h-6 mr-4">
        <input
          ...
          :class="
            twMerge(
              'peer h-5 w-5 appearance-none rounded-full border-2 border-border-strong',
          ...
          @change="onChange" />
        <div
          class="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 hidden peer-checked:block">
          <div class="w-2 h-2 rounded-full bg-inverse" />
        </div>
      </div>

      <div
        v-if="$slots.icon"
        :class="`mr-4 ${isBoxed ? 'mt-0.5' : ''} text-foreground-muted`">
        <slot name="icon" />
      </div>
