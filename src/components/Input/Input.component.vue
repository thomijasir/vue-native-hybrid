...
<template>
  <div :class="twMerge('flex flex-col', fullWidth ? 'w-full' : '', containerClass)">
    <label
      v-if="label"
      :for="inputId"
      :class="twMerge('mb-1.5 text-sm font-medium text-foreground', disabled ? 'opacity-50' : '')"
    >
      {{ label }}
    </label>
    ...
    <div class="relative">
      <div
        :class="twMerge(
          ...
          disabled ? 'opacity-50 cursor-not-allowed bg-surface-disabled' : 'cursor-pointer',
        )"
    ...
    <div v-if="helperText || (isError && typeof error === 'string')" class="mt-1 text-xs">
